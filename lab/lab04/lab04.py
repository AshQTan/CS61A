...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    gotham = make_city('gotham', lat, lon)
    if distance(gotham, city1)>distance(gotham, city2):
        return city2[0]
    return city1[0]
...
def put_piece(board, max_rows, column, player):
    ...
    while x>-1:
        if get_piece(board, x, column) == '-':
            row_index=x
            new_row=replace_elem(board[x], column, player)
            new_board=replace_elem(board, x, new_row)
            return x, new_board
        x-=1
    return -1, new_board
