...
(define (sub-all s olds news)
  ...
  ; (cond ((null? s) nil)
  ;   ((list? (car s)) (cons (substitute (car s) (car old) (car new))) (sub-all (cdr s) (cdr olds) (cdr news)))
  ;   ((equal? (car s) (car old)) (cons new (substitute (cdr s) old new)))
  ;   (else (cons (car s) ))
  ;   )
)
...
(define (exp? exp)
  (if (and (list? exp) (eq? (car exp) '^) (= (length exp) 3) True) True
    False 
    )
  )
