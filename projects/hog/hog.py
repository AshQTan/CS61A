...
def reroll(dice):
    ...
    def rerolled():
        ...
        d = dice()
        if (d%2==0):
            return d  # Replace this statement
        else: 
            return dice()
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    def player_scores(player):  #returns score of a specific player
        if player == 0:
            return score0, score1
        return score1, score0

    def player_strats(player):   #returns strategy of specific player
        if player == 0:
            return strategy0, strategy1
        return strategy1, strategy0

    def add_score(player, turn):  #add turn's score to player's total score
    ...
    while score0 < goal and score1 < goal:
        curr_score, opp_score = player_scores(player)
        curr_strat, opp_strat = player_strats(player)
        dice = select_dice(curr_score, opp_score, dice_swapped)
        if curr_strat(curr_score, opp_score)== -1:
            add_score(player, 1)
            dice_swapped = not dice_swapped
        else: 
            add_score(player, take_turn(curr_strat(curr_score, opp_score), opp_score, dice))

        if (score0 == score1 *2) or (score1 == score0*2): #swine function
...
def check_strategy(strategy, goal=GOAL_SCORE):
    ...
    for y in range (0, goal):
        for z in range (0, goal):
            if strategy(y, z)< -1 or strategy(y, z)>10:
                return check_strategy_roll(y, z, strategy(y, z))
    return None
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, num_samples=100):
    ...
    avg_max, most = 0, 0
    for r in range (1, 11):
        avg = make_averaged(roll_dice)(r, dice)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if opponent_score == (score*2)+1:
        return 0
    elif score + current_score == 2*opponent_score:
        return num_rolls
    else:
