...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
	...
	if scheme_symbolp(first) and first in SPECIAL_FORMS:
		...
		return scheme_apply(proc, arguments, env)
...
class Frame:
	...
	def make_child_frame(self, formals, vals):
		...
		if len(formals) != len(vals):
			raise SchemeError("not equal lengths")

		def helper(form, val, env):
			if form is not nil and val is not nil:
				env.define(form.first, val.first)
				return helper(form.second, val.second, env)
			else:
				return
		helper(formals, vals, child)
...
class PrimitiveProcedure(Procedure):
	...
	def apply(self, args, env):
		...
		try:
			...
			raise SchemeError("something went wrong, sort of like this election")
...
def do_define_form(expressions, env):
	...
	if scheme_symbolp(target):
		...
		# BEGIN PROBLEM 6
		#def_val = scheme_eval(expressions.second, env)
...
def do_lambda_form(expressions, env):
	...
	if func is nil:
		return LambdaProcedure(args, Pair("begin", func), env)
	return LambdaProcedure(args, func, env)
	# END PROBLEM 9
...
def make_let_frame(bindings, env):
	...
	while bindings is not nil:
		if bindings.first.second.second is not nil:
			raise SchemeError("why")
		if type(bindings.first.first) is int or type(bindings.first.first) is float:
			raise SchemeError("whyv2")
		form = Pair(bindings.first.first, form)
		vals = Pair(scheme_eval(bindings.first.second.first, env), vals)
		bindings = bindings.second
	if len(form) != len(vals):
		raise SchemeError("whoops")
	return env.make_child_frame(form, vals)
...
def scheme_optimized_eval(expr, env, tail=False):
	...
	while isinstance(result, Thunk):
		...
		if (scheme_symbolp(first) and first in SPECIAL_FORMS):
			...
			argument = rest.map(lambda operand: scheme_eval(operand, env))
			result = scheme_apply(procedure, argument, env)
			# END Extra Credit
	return result
